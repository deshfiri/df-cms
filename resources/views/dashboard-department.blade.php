...
<div class="row g-3 mb-3">
    <div class="col-6 col-md-3">
        <div class="card text-center py-3">
            ...
            <div style="font-size:.69rem;color:var(--text3);text-transform:uppercase;letter-spacing:.04em">Awaiting Your Team</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="card text-center py-3">
            <div class="fw-bold fs-4 mb-0" style="color:var(--primary)">{{ $pending->where('status', 'Submitted')->count() }}</div>
            <div style="font-size:.69rem;color:var(--text3);text-transform:uppercase;letter-spacing:.04em">Awaiting Approval</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="card text-center py-3">
            <div class="fw-bold fs-4 mb-0" style="color:var(--primary)">{{ $myTasks->count() }}</div>
            <div style="font-size:.69rem;color:var(--text3);text-transform:uppercase;letter-spacing:.04em">My Open Tasks</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="card text-center py-3">
            <div class="fw-bold fs-4 mb-0 c-red">{{ $overdueTaskCount }}</div>
            <div style="font-size:.69rem;color:var(--text3);text-transform:uppercase;letter-spacing:.04em">Overdue Tasks</div>
        </div>
    </div>
</div>
...
<div class="row g-3">
    <div class="col-lg-7">
        <div class="card section-card">
            <div class="card-header py-3"><h6 class="fw-bold mb-0">Clients Waiting on {{ $departments->implode(' / ') }}</h6></div>
            <div class="card-body p-0">
                @forelse($pending as $row)
                <a href="{{ route('clients.show', $row->client_id) }}" class="d-flex align-items-center gap-3 p-3 text-decoration-none" style="border-bottom:1px solid var(--border)">
                    <div class="flex-grow-1">
                        <div class="fw-semibold small" style="color:var(--text)">{{ $row->client->client_name ?? '—' }}</div>
    ...
    <div class="col-lg-5">
        <div class="card section-card">
            <div class="card-header py-3"><h6 class="fw-bold mb-0">My Tasks</h6></div>
            <div class="card-body p-0">
                ...
                <div class="d-flex align-items-center gap-3 p-3" style="border-bottom:1px solid var(--border)">
                    <div class="flex-grow-1">
                        ...
                        <div style="font-size:.72rem;color:var(--text3)">{{ $task->client->client_name ?? '—' }} · Due {{ $task->due_date?->format('d M Y') ?? '—' }}</div>
                    </div>
                    <span class="spill {{ $task->is_overdue ? 'spill-rejected' : 'spill-pending' }}">{{ $task->is_overdue ? 'Overdue' : $task->status }}</span>
                </div>
                @empty
                <div class="text-center py-5" style="color:var(--text3)">
