...
<script>
    $(function () {
        if ($('#histTable').length) {
            $('#histTable').DataTable({ order: [], paging: true, pageLength: 25, lengthChange: false, info: false, language: { search: '', searchPlaceholder: 'Search employee…' }, columnDefs: [{ orderable: false, targets: [10] }] });
        }
    });
</script>
@endpush
