...
<head>
    ...
    :root {
        ...
        --bs-primary-rgb:      {{ $tR }}, {{ $tG }}, {{ $tB }};
        --bs-link-color:       {{ $themeColor }};
        --bs-link-hover-color: {{ $themeColorDark }};
        --bs-link-color-rgb:   {{ $tR }}, {{ $tG }}, {{ $tB }};
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    [data-theme="dark"] #sidebar { background: #080e1a; border-right: 1px solid #1a2a38; }
    [data-theme="dark"] .sb-brand { border-bottom-color: #1a2a38; }
    ...
    [data-theme="dark"] .sb-link.active i { color: #3B82F6; }
    [data-theme="dark"] .sb-section { color: rgba(255,255,255,.3); letter-spacing: .1em; }
    ...
    [data-theme="dark"] .table tbody td { border-color: rgba(255,255,255,.05) !important; }
    ...
    [data-theme="dark"] .dropdown-item:hover { background: #1a2a3e; color: var(--text); }
    ...
    [data-theme="dark"] .modal-footer { background: #0e1929; border-color: #1f2d40; }
    ...
    [data-theme="dark"] .qv-info-box { background: #1a2235; border: 1px solid #1f2d40; }
    ...
    [data-theme="dark"] .fpill:hover { border-color: var(--c-blue); color: var(--c-blue); background: rgba(59,130,246,.08); }
    [data-theme="dark"] .fpill.active { background: var(--c-blue); border-color: var(--c-blue); color: #fff; }

    /* ── Dark mode quick view backdrop ──────────────────────────── */
    [data-theme="dark"] #qvBackdrop { background: rgba(0,0,0,.6); }

    /* ── Dark mode bulk bar ─────────────────────────────────────── */
    [data-theme="dark"] #bulkBar { background: #141e2e; border: 1px solid #2a3d55; color: var(--text); }

    /* ── Dark mode form controls ─────────────────────────────────── */
    [data-theme="dark"] .form-control,
    [data-theme="dark"] .form-select { background: #1a2235 !important; border-color: #2a3d55 !important; color: var(--text) !important; }
    [data-theme="dark"] .form-control:focus,
    [data-theme="dark"] .form-select:focus { background: #1f2a40 !important; border-color: var(--c-blue) !important; box-shadow: 0 0 0 3px rgba(59,130,246,.2) !important; }
    [data-theme="dark"] .input-group-text { background: #141e2e !important; border-color: #2a3d55 !important; color: var(--text3); }
    [data-theme="dark"] div.dataTables_wrapper .dataTables_length select,
    [data-theme="dark"] div.dataTables_wrapper .dataTables_filter input { background: #1a2235 !important; border-color: #2a3d55 !important; }
    [data-theme="dark"] .select2-container--bootstrap-5 .select2-selection { background: #1a2235 !important; border-color: #2a3d55 !important; }
    [data-theme="dark"] .select2-dropdown { background: #141e2e !important; border-color: #2a3d55 !important; }
    [data-theme="dark"] .select2-search--dropdown .select2-search__field { background: #1a2235 !important; border-color: #2a3d55 !important; }
    [data-theme="dark"] .swal2-popup { background: #111827 !important; border: 1px solid #2a3d55 !important; }
    [data-theme="dark"] .swal2-input { background: #1a2235 !important; border-color: #2a3d55 !important; color: var(--text) !important; }

    /* ── Bootstrap theme overrides ──────────────────────────────── */
    .btn-primary {
        --bs-btn-bg:                  {{ $themeColor }};
        --bs-btn-border-color:        {{ $themeColor }};
        --bs-btn-hover-bg:            {{ $themeColorDark }};
        --bs-btn-hover-border-color:  {{ $themeColorDark }};
        --bs-btn-active-bg:           {{ $themeColorDark }};
        --bs-btn-active-border-color: {{ $themeColorDark }};
        --bs-btn-disabled-bg:         {{ $themeColor }};
        --bs-btn-disabled-border-color: {{ $themeColor }};
        --bs-btn-focus-shadow-rgb:    {{ $tR }}, {{ $tG }}, {{ $tB }};
    }
    .btn-outline-primary {
    ...
    .sb-brand-name { font-size: .95rem; font-weight: 700; color: #fff; line-height: 1.2; }
    .sb-brand-sub  { font-size: .72rem; color: var(--sb-text); }
    .sb-brand-logo { max-height: 30px; max-width: 120px; object-fit: contain; flex-shrink: 0; }
    .sb-brand-text { overflow: hidden; transition: opacity var(--t), width var(--t); }

    .sb-nav { padding: 8px 7px; flex: 1; overflow-y: auto; overflow-x: hidden; }
    .sb-nav::-webkit-scrollbar { width: 2px; }
    .sb-nav::-webkit-scrollbar-thumb { background: var(--sb-bd); }
    ...
    .sb-link .sb-lbl { overflow: hidden; transition: opacity var(--t), width var(--t); }

    #sidebar.collapsed { width: var(--sidebar-mini); }
    #sidebar.collapsed .sb-brand-text { opacity: 0; width: 0; }
    #sidebar.collapsed .sb-brand-logo { max-width: 32px; }
    #sidebar.collapsed .sb-section { opacity: 0; height: 0; padding-top: 0; padding-bottom: 0; overflow: hidden; }
    #sidebar.collapsed .sb-link { justify-content: center; padding: 9px 0; }
    #sidebar.collapsed .sb-link .sb-lbl { opacity: 0; width: 0; }
    ...
    #topbar.collapsed { left: var(--sidebar-mini); }
    ...
    .tb-search-box i { color: var(--text3); font-size: .83rem; flex-shrink: 0; }
    .tb-search-box input {
        border: none; background: transparent; outline: none;
        font-size: .82rem; color: var(--text); flex: 1; font-family: inherit;
    }
    .tb-search-box input::placeholder { color: var(--text3); }
    .tb-kbd {
    ...
    #main.collapsed { margin-left: var(--sidebar-mini); }
    ...
    .section-card { border-radius: var(--radius) !important; }
    ...
    .table tbody td {
        ...
        vertical-align: middle; border-color: var(--border) !important;
        color: var(--text);
    }
    ...
    /* ── Alerts ─────────────────────────────────────────────────── */
    .alert { border-radius: var(--radius); border: none; font-size: .79rem; }
    [data-theme="dark"] .alert-success { background: var(--c-green-bg);  color: var(--c-green); }
    [data-theme="dark"] .alert-danger  { background: var(--c-red-bg);    color: var(--c-red); }
    [data-theme="dark"] .alert-warning { background: var(--c-yellow-bg); color: var(--c-yellow); }
    [data-theme="dark"] .alert-info    { background: var(--c-blue-bg);   color: var(--c-blue); }

    /* ── Forms ──────────────────────────────────────────────────── */
    .form-control, .form-select {
        background: var(--surface); border-color: var(--border);
        color: var(--text); font-size: .79rem; border-radius: 7px;
        font-family: inherit;
    }
    ...
    /* ── Buttons ────────────────────────────────────────────────── */
    .btn { font-size: .77rem; font-weight: 500; border-radius: 7px; }
    ...
    /* ── Badges ─────────────────────────────────────────────────── */
    .badge { font-size: .65rem; font-weight: 600; letter-spacing: .01em; border-radius: 20px; padding: 3px 7px; }
    ...
    .spill::before { content:''; width:5px; height:5px; border-radius:50%; flex-shrink:0; background:currentColor; opacity:.7; }
    .spill-running   { background: var(--c-green-bg);  color: var(--c-green); }
    .spill-warning   { background: var(--c-yellow-bg); color: var(--c-yellow); }
    .spill-completed { background: var(--c-blue-bg);   color: var(--c-blue); }
    .spill-hold      { background: var(--c-slate-bg);  color: var(--c-slate); }
    .spill-cancelled { background: var(--c-red-bg);    color: var(--c-red); }

    /* Workflow stage statuses */
    .spill-pending       { background: var(--c-slate-bg);  color: var(--c-slate); }
    .spill-in-progress   { background: var(--c-blue-bg);   color: var(--c-blue); }
    .spill-submitted     { background: var(--c-purple-bg); color: var(--c-purple); }
    .spill-need-revision { background: var(--c-yellow-bg); color: var(--c-yellow); }
    .spill-approved      { background: var(--c-green-bg);  color: var(--c-green); }
    .spill-rejected      { background: var(--c-red-bg);    color: var(--c-red); }
    .spill-locked        { background: var(--c-slate-bg);  color: var(--c-slate); opacity: .7; }
    .spill-secondary     { background: var(--secondary-bg); color: var(--secondary); }
    .spill-blocked       { background: var(--c-red-bg);    color: var(--c-red); }
    .spill-delayed       { background: var(--c-yellow-bg); color: var(--c-yellow); }
    ...
    .kpi-val { font-size: 1.5rem; font-weight: 700; letter-spacing: -.04em; color: var(--text); line-height: 1; }
    .kpi-lbl { font-size: .67rem; font-weight: 500; color: var(--text3); margin-top: 4px; text-transform: uppercase; letter-spacing: .04em; }
    .kpi-icon { width: 33px; height: 33px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: .88rem; }
    ...
    .fpill.active { background: var(--primary); border-color: var(--primary); color: #fff; }
    .fpill .fcnt { font-size: .62rem; opacity: .75; }
    ...
    #qvBackdrop.show { opacity: 1; pointer-events: all; }
    #qvDrawer {
    ...
    #qvDrawer.open { right: 0; }
    .qv-header {
        padding: 12px 14px; border-bottom: 1px solid var(--border);
        display: flex; align-items: flex-start; gap: 10px; flex-shrink: 0;
    }
    .qv-body { flex: 1; overflow-y: auto; padding: 16px; }
    .qv-body::-webkit-scrollbar { width: 3px; }
    .qv-body::-webkit-scrollbar-thumb { background: var(--border); }
    .qv-avatar {
    ...
    .qv-sec { margin-bottom: 16px; }
    .qv-sec-title {
        font-size: .63rem; font-weight: 600; text-transform: uppercase;
        letter-spacing: .06em; color: var(--text3); margin-bottom: 7px;
    }
    .qv-info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; }
    .qv-info-box { background: var(--surface2); border-radius: 7px; padding: 8px 10px; }
    .qv-info-lbl { font-size: .63rem; color: var(--text3); font-weight: 500; }
    .qv-info-val { font-size: .77rem; color: var(--text); font-weight: 500; margin-top: 1px; }

    /* ── Bulk action bar ────────────────────────────────────────── */
    #bulkBar {
        position: fixed; bottom: -80px; left: 50%; transform: translateX(-50%);
        background: var(--text); color: var(--surface);
        padding: 8px 16px; border-radius: 12px; box-shadow: var(--shadow-lg);
    ...
    #bulkBar .bb-sep { width: 1px; height: 16px; background: rgba(255,255,255,.15); flex-shrink: 0; }

    /* ── Timeline ───────────────────────────────────────────────── */
    .tl { padding-left: 20px; position: relative; }
    .tl::before { content:''; position:absolute; left:5px; top:8px; bottom:4px; width:1px; background:var(--border); }
    .tl-item { position: relative; padding-bottom: 13px; }
    .tl-dot { position:absolute; left:-19px; top:3px; width:8px; height:8px; border-radius:50%; background:var(--border); border:2px solid var(--surface); }
    .tl-dot.done { background: var(--primary); }

    /* ── Progress ───────────────────────────────────────────────── */
    .progress { height: 4px; background: var(--border); border-radius: 20px; overflow: visible; }
    ...
    div.dataTables_wrapper .dataTables_filter input {
        ...
        border-radius: 6px; font-size: .77rem; font-family: inherit;
    }
    /* dataTables dark handled in consolidated block */
    div.dataTables_wrapper .dataTables_info { font-size: .72rem; color: var(--text3); }
    div.dataTables_wrapper .dataTables_paginate .paginate_button {
        border-radius: 6px !important; font-size: .76rem;
    ...
    .dropdown-menu {
        background: var(--surface); border: 1px solid var(--border);
        border-radius: 10px; box-shadow: var(--shadow-lg); font-size: .79rem;
    }
    .dropdown-item { color: var(--text2); border-radius: 6px; padding: 6px 11px; }
    .dropdown-item:hover { background: var(--surface2); color: var(--text); }
    .dropdown-header { color: var(--text3); font-size: .66rem; font-weight: 600; letter-spacing: .05em; text-transform: uppercase; }
    .dropdown-divider { border-color: var(--border); }

    /* ── Modals ─────────────────────────────────────────────────── */
    .modal-content { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; }
    .modal-header  { border-color: var(--border); background: var(--surface2); border-radius: 12px 12px 0 0; }
    .modal-footer  { border-color: var(--border); background: var(--surface2); border-radius: 0 0 12px 12px; }
    .modal-body    { color: var(--text); }
    .modal-title   { font-size: .88rem; font-weight: 600; color: var(--text); }
    .btn-close     { filter: none; }
    [data-theme="dark"] .btn-close { filter: invert(1); }

    /* ── Typography ─────────────────────────────────────────────── */
    .page-title { font-size: var(--fs-h4); font-weight: var(--fw-bold); color: var(--text); letter-spacing: -.02em; }
    .section-title { font-size: var(--fs-h3); font-weight: var(--fw-bold); color: var(--text); letter-spacing: -.01em; }
    ...
    [data-theme="dark"] .select2-container--bootstrap-5 .select2-selection__rendered { color: var(--text); }
    [data-theme="dark"] .select2-container--bootstrap-5 .select2-selection__placeholder { color: var(--text3); }
    [data-theme="dark"] .select2-results__option { color: var(--text); }
    [data-theme="dark"] .select2-results__option--highlighted[aria-selected] { background: var(--primary) !important; }
    [data-theme="dark"] .swal2-title { color: var(--text); }
    ...
    [data-theme="dark"] .badge.bg-light   { background: var(--surface2) !important; color: var(--text2) !important; border-color: var(--border) !important; }
    [data-theme="dark"] .form-check-input { background-color: var(--surface2); border-color: var(--border); }
    [data-theme="dark"] .form-check-input:checked { background-color: var(--primary); border-color: var(--primary); }
    [data-theme="dark"] .table-hover > tbody > tr:hover > * { --bs-table-hover-bg: var(--surface2); --bs-table-hover-color: var(--text); }
    [data-theme="dark"] .nav-tabs         { border-color: var(--border); }
    [data-theme="dark"] .nav-tabs .nav-link { color: var(--text2); }
    ...
    [data-theme="dark"] .text-success  { color: var(--c-green)  !important; }
    [data-theme="dark"] .text-danger   { color: var(--c-red)    !important; }
    [data-theme="dark"] .text-warning  { color: var(--c-yellow) !important; }
    [data-theme="dark"] .text-info     { color: var(--c-blue)   !important; }
    ...
    [data-theme="dark"] .bg-success.bg-opacity-25  { background-color: var(--c-green-bg) !important; }
    ...
    [data-theme="dark"] .bg-danger.bg-opacity-25   { background-color: var(--c-red-bg)   !important; }
    ...
    [data-theme="dark"] .bg-warning.bg-opacity-25  { background-color: var(--c-yellow-bg) !important; }
    [data-theme="dark"] .bg-info,
    [data-theme="dark"] .bg-info.bg-opacity-10     { background-color: var(--c-blue-bg)  !important; }
    [data-theme="dark"] .bg-secondary              { background-color: var(--surface2)   !important; }

    /* Bootstrap contextual badges */
    [data-theme="dark"] .badge.bg-success  { color: var(--c-green)  !important; }
    [data-theme="dark"] .badge.bg-danger   { color: var(--c-red)    !important; }
    [data-theme="dark"] .badge.bg-warning  { color: var(--c-yellow) !important; }
    [data-theme="dark"] .badge.bg-info     { color: var(--c-blue)   !important; }
    [data-theme="dark"] .badge.bg-secondary { color: var(--text2)   !important; border: 1px solid var(--border); }
    [data-theme="dark"] .badge.bg-primary   { color: #fff; }

    /* Bootstrap contextual border in dark mode */
    [data-theme="dark"] .border-success { border-color: var(--c-green) !important; }
    [data-theme="dark"] .border-danger  { border-color: var(--c-red)   !important; }
    [data-theme="dark"] .border-warning { border-color: var(--c-yellow)!important; }

    /* Progress bars need solid fills, not semi-transparent */
    [data-theme="dark"] .progress-bar.bg-success { background-color: var(--c-green)  !important; }
    [data-theme="dark"] .progress-bar.bg-warning { background-color: var(--c-yellow) !important; }
    [data-theme="dark"] .progress-bar.bg-danger  { background-color: var(--c-red)    !important; }
    [data-theme="dark"] .progress-bar.bg-info    { background-color: var(--c-blue)   !important; }

    [data-theme="dark"] .table-light    { --bs-table-bg: var(--surface2) !important; color: var(--text) !important; }
    ...
    .nav-tabs .nav-link { color: var(--text2); border-radius: 6px 6px 0 0; font-size: .77rem; padding: 7px 14px; font-weight: 500; border: 1px solid transparent; }
    .nav-tabs .nav-link:hover { color: var(--text); border-color: var(--border) var(--border) transparent; }
    .nav-tabs .nav-link.active { background: var(--surface); border-color: var(--border) var(--border) var(--surface); color: var(--primary); font-weight: 600; }

    /* ── Empty state ────────────────────────────────────────────── */
    .empty-state { text-align: center; padding: 40px 20px; color: var(--text3); }
    .empty-state i { font-size: 2.2rem; margin-bottom: 10px; display: block; }
    .empty-state p { font-size: .79rem; margin: 4px 0 0; color: var(--text3); }
    ...
    .c-slate  { color: var(--c-slate)  !important; }
    .c-rose   { color: var(--c-rose, #F43F5E) !important; }
    ...
    @media (max-width: 991.98px) {
        ...
        #topbar, #topbar.collapsed { left: 0; }
        #main, #main.collapsed { margin-left: 0; padding: 14px; }
        #qvDrawer { width: 100%; right: -100%; }
        #sidebarOverlay {
...
<aside id="sidebar">
    ...
    <nav class="sb-nav">
        ...
        <a href="{{ route('dashboard') }}" class="sb-link {{ request()->routeIs('dashboard') ? 'active' : '' }}" title="Dashboard" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-speedometer2"></i><span class="sb-lbl">Dashboard</span>
        </a>
        <a href="{{ route('clients.index') }}" class="sb-link {{ request()->routeIs('clients.*') ? 'active' : '' }}" title="Clients" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-people"></i><span class="sb-lbl">Clients</span>
        </a>

        <a href="{{ route('meetings.all') }}" class="sb-link {{ request()->routeIs('meetings.all') ? 'active' : '' }}" title="All Meetings" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-calendar-event"></i><span class="sb-lbl">Meetings</span>
        </a>
        <a href="{{ route('meetings.book') }}" class="sb-link {{ request()->routeIs('meetings.book') ? 'active' : '' }}" title="Book a Meeting" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-calendar-plus"></i><span class="sb-lbl">Book Meeting</span>
        </a>
        @can('view tasks')
        <a href="{{ route('tasks.index') }}" class="sb-link {{ request()->routeIs('tasks.*') ? 'active' : '' }}" title="Tasks" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-list-check"></i><span class="sb-lbl">Tasks</span>
        </a>
        @endcan

        <div class="sb-section">Operations</div>
        <a href="{{ route('import.index') }}" class="sb-link {{ request()->routeIs('import.*') ? 'active' : '' }}" title="Import Data" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-upload"></i><span class="sb-lbl">Import Data</span>
        </a>
        <a href="#" id="exportSidebarBtn" class="sb-link" title="Export Data" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-download"></i><span class="sb-lbl">Export Data</span>
        </a>
        <a href="{{ route('workflow.index') }}" class="sb-link {{ request()->routeIs('workflow.*') ? 'active' : '' }}" title="Workflow Stages" data-bs-toggle="tooltip" data-bs-placement="right">
        ...
        <a href="{{ route('categories.index') }}" class="sb-link {{ request()->routeIs('categories.*') ? 'active' : '' }}" title="Categories" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-tags"></i><span class="sb-lbl">Categories</span>
        </a>
        @endcan
        @can('manage users')
        <a href="{{ route('users.index') }}" class="sb-link {{ request()->routeIs('users.*') ? 'active' : '' }}" title="Users" data-bs-toggle="tooltip" data-bs-placement="right">
        ...
        <a href="{{ route('roles.index') }}" class="sb-link {{ request()->routeIs('roles.*','permissions.*') ? 'active' : '' }}" title="Roles & Permissions" data-bs-toggle="tooltip" data-bs-placement="right">
            <i class="bi bi-shield-lock"></i><span class="sb-lbl">Roles & Permissions</span>
        </a>
        <a href="{{ route('settings.index') }}" class="sb-link {{ request()->routeIs('settings.*') ? 'active' : '' }}" title="Settings" data-bs-toggle="tooltip" data-bs-placement="right">
...
<header id="topbar">
    ...
    <div class="ms-auto d-flex align-items-center gap-2">
        <button id="darkToggle" class="btn btn-sm d-flex align-items-center justify-content-center"
                style="width:35px;height:35px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--text2);padding:0" title="Toggle theme">
            <i id="darkIcon" class="bi bi-moon-stars" style="font-size:.88rem"></i>
        </button>

        <div class="dropdown">
            <button class="btn btn-sm d-flex align-items-center justify-content-center position-relative" data-bs-toggle="dropdown" id="notifBell"
                    style="width:35px;height:35px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--text2);padding:0" title="Notifications">
                <i class="bi bi-bell" style="font-size:.88rem"></i>
                <span id="notifBadge" class="d-none position-absolute" style="top:-4px;right:-4px;background:var(--c-red);color:#fff;border-radius:20px;font-size:.6rem;font-weight:700;padding:1px 5px;line-height:1.3">0</span>
            </button>
            <div class="dropdown-menu dropdown-menu-end mt-1 p-0" style="min-width:320px;max-height:420px;overflow-y:auto">
                <div class="d-flex align-items-center justify-content-between px-3 py-2" style="border-bottom:1px solid var(--border)">
                    ...
                    <button id="notifMarkAll" class="btn btn-sm p-0" style="font-size:.68rem;color:var(--primary);background:none;border:none">Mark all read</button>
                </div>
                <div id="notifList"><div class="text-center py-4"><div class="spinner-border spinner-border-sm" style="color:var(--primary)"></div></div></div>
            </div>
        </div>

        @can('manage clients')
        <a href="{{ route('clients.create') }}" class="btn btn-primary btn-sm d-none d-md-flex align-items-center gap-1" style="height:33px;padding:0 12px;font-size:.78rem">
            <i class="bi bi-plus-lg"></i><span>New Client</span>
        </a>
        @endcan

        <div class="dropdown">
            <button class="btn p-0 d-flex align-items-center gap-2" data-bs-toggle="dropdown" style="background:none;border:none">
                <div class="user-avatar">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</div>
                <span class="d-none d-md-block" style="font-size:.76rem;font-weight:500;color:var(--text2)">{{ Str::limit(auth()->user()->name, 18) }}</span>
            ...
            <ul class="dropdown-menu dropdown-menu-end mt-1" style="min-width:200px">
                <li class="px-3 py-2">
                    ...
                    <span class="badge mt-1 me-1" style="background:rgba(var(--primary-rgb),.12);color:var(--primary);font-size:.6rem">{{ $roleName }}</span>
                    @endforeach
                </li>
                ...
                <li>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="dropdown-item" style="color:#ef4444"><i class="bi bi-box-arrow-right me-2"></i>Sign out</button>
                    </form>
                </li>
            </ul>
        </div>
...
<main id="main">
    @if(session('success'))
    <div class="alert alert-success alert-dismissible fade show py-2 mb-3 d-flex align-items-center gap-2" role="alert">
    ...
    @endif
    @if(session('error'))
    <div class="alert alert-danger alert-dismissible fade show py-2 mb-3 d-flex align-items-center gap-2" role="alert">
...
<aside id="qvDrawer">
    <div class="qv-header">
        ...
        <div class="flex-fill min-w-0">
            <div style="font-size:.88rem;font-weight:600;color:var(--text)" class="text-truncate" id="qvName">Loading…</div>
            <div class="d-flex align-items-center gap-2 mt-1 flex-wrap">
                <span id="qvDfid" class="badge" style="background:var(--surface2);color:var(--text2);border:1px solid var(--border);font-size:.62rem"></span>
                <span id="qvStatus"></span>
            </div>
        </div>
        <div class="d-flex gap-1 ms-2 flex-shrink-0">
            <a id="qvEditLink" href="#" class="btn btn-sm btn-outline-secondary px-2" title="Edit"><i class="bi bi-pencil"></i></a>
            <button onclick="closeDrawer()" class="btn btn-sm btn-outline-secondary px-2"><i class="bi bi-x-lg"></i></button>
        </div>
    </div>
...
<div id="bulkBar">
    ...
    <button id="bulkDeleteBtn" class="btn btn-sm" style="background:rgba(239,68,68,.18);color:#f87171;border:none;padding:3px 10px">
        <i class="bi bi-trash me-1"></i>Delete
    </button>
    <button onclick="$('#selectAll').prop('checked',false).trigger('change')" class="btn btn-sm" style="background:rgba(255,255,255,.08);color:rgba(255,255,255,.7);border:none;padding:3px 10px">
        Cancel
    </button>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body px-3 py-3">
                ...
                <div class="form-check mb-2">
                    <input class="form-check-input" type="radio" name="exportFmt" value="{{ $val }}" id="fmt{{ $val }}" {{ $val==='excel' ? 'checked' : '' }}>
            ...
            <div class="modal-footer py-2 px-3">
                ...
                <a href="#" id="doExport" class="btn btn-sm btn-primary"><i class="bi bi-download me-1"></i>Download</a>
            </div>
        </div>
    </div>
</div>
    ...
    .then(r => {
        if (r.isConfirmed) {
            $.post('{{ route("clients.bulk-delete") }}', { ids })
             .done(res => { Swal.fire('Deleted', res.message, 'success'); if(window.table) window.table.ajax.reload(); });
        }
    });
});
