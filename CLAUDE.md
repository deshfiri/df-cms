...
**Task and workflow attachments stage through the queue.** `App\Services\Storage\UploadStaging` writes them to `local` first and queues `PushUploadToProvider`, which copies each file to the active provider and only then repoints the record. The request never waits on the CDN, and the record always names the disk the file is really on, so both rules above still hold. With the `sync` queue it writes straight to `activeDisk()` as before. Without a running worker, staged files simply stay local and keep working; `storage:status` reports how many are waiting. In the browser, `partials/upload-queue.blade.php` (`makeUploadQueue`) sends bulk uploads one file per request, two at a time, and refuses files over `App\Support\UploadLimit` (the app's limit capped by PHP's) before sending them.
